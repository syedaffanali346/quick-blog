...
const AddBlog = () => {
  ...
  const generateContent = async () => {
      if (!title) {
        toast.error("Fill title field for generating description!")
      }
      try {
        setLoading(true)
        const {data} = await axios.post("/api/blog/generatedesc", {prompt: title})
        console.log(data)
        if(data.success){
          quillRef.current.root.innerHTML = parse(data.description)
        }
        else{
          toast.error(data.message)
        }
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 px-5 sm:pt-12 sm:pl-16 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:p-10 shadow rounded'>

        <p>Upload Thumbnail</p>
        <label htmlFor="image">
          <img src={image ? URL.createObjectURL(image) :assets.upload_area} alt="upload" className='mt-2 h-16 rounded cursor-pointer'/>
        ...
        <p>Blog Subtitle</p>
        <input type="text" placeholder='Type here..' className='w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded' onChange={(e)=> setSubTitle(e.target.value)} value={subTile} />
